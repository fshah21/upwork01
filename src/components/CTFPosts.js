...
const CTFPosts = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await fetch('https://public-api.wordpress.com/rest/v1.1/sites/example90078.wordpress.com/posts?category=ctf');
        if (!response.ok) {
  ...
  return (
    <div className="text-gray-400 bg-gray-900 body-font h-screen w-screen">
        ...
        <div key={post.ID} className="lg:w-4/5 md:w-4/5 flex flex-col mx-auto items-center bg-gray-800 border border-gray-800 rounded-lg shadow md:flex-row dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 dark:bg-slate-800 m-10">
            <img className="rounded-t-lg lg:h-2/4 lg:w-2/4 p-4 max-h-64 max-w-full" src={post.featured_image} alt="upwork"/>
            <div className="flex flex-col justify-between p-10 leading-normal" key={post.ID} onClick={() => handleClick(post.ID)}>
